...
3. 暂存前检查已修改和未跟踪文件：
   ...
   git status --short
   ```
4. 检查是否存在密钥或 AI/工具生成文件。
5. 可行时执行对应后端、前端或部署验证。
6. 只暂存本次任务需要提交的文件。
7. 使用 `type(scope): subject` 提交；当 Codex 实际参与时追加协作署名。
8. 用户要求推送时，使用 `git push -u origin HEAD`。
9. 用户要求创建 PR 时，使用 `gh pr create`。
...
不要为宽泛模式静默改写 `.gitignore`。如果发现新的生成文件或目录应被忽略，先展示建议添加项；只有明显安全或用户确认后才写入。
