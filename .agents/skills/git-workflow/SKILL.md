...
1. 检查当前状态：
   ...
   ```
2. 提交前必须确认 GitHub CLI 已安装并登录：
   ```powershell
   gh --version
   gh auth status
   ```
   如果 `gh` 未安装或未登录，停止流程，提示用户先安装并登录；AI 不得继续提交、推送或创建 PR。
3. 如果要新建功能分支，并且工作区干净，先同步默认分支：
   ```powershell
   git checkout main
   git pull origin main
   git checkout -b codex/YYYYMMDD_task-summary
   ```
4. 除非用户明确要求“直接提交到 main”，否则不要在 `main` 上提交业务或文档改动；应创建 `codex/YYYYMMDD_task-summary` 分支并推送该分支。
5. 暂存前检查已修改和未跟踪文件：
   ```powershell
   git diff --stat
   git status --short
   ```
6. 检查是否存在密钥或 AI/工具生成文件。
7. 可行时执行对应后端、前端或部署验证。
8. 只暂存本次任务需要提交的文件。
9. 使用 `type(scope): subject` 提交；当 Codex 实际参与时追加协作署名。
10. 推送当前功能分支：
   ```powershell
   git push -u origin HEAD
   ```
11. 使用 `gh pr create` 创建 PR，并在交付中给出 PR 链接。除非用户明确要求直接提交到主干，否则流程必须以 PR 作为交付终点。
...
1. `gh` 未安装：停止流程，提示用户安装 GitHub CLI 后重新发起提交/PR 请求。
2. `gh` 未登录：停止流程，提示用户执行 `gh auth login` 后重新发起提交/PR 请求。
3. `gh` 已安装且已登录：可以继续提交、推送，并必须由 AI 使用 `gh pr create` 创建 PR。
4. 如果用户只要求本地提交但没有要求 PR，也仍需先确认 `gh` 可用；不可用时不提交代码，避免产生无法完成标准流程的半成品提交。
