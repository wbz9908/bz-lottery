...
本文件是 bz-lottery 仓库的 Codex 根入口。以后在项目根目录打开 Codex 会话时，先读取本文件，再按任务类型读取 `.agents/skills/` 下的对应 Skill。
...
1. 先读 Skill，再读 Skill 中列出的 `references/` 文档，然后再修改代码或配置。
...
6. 修改后按 Skill 要求执行验证；如无法验证，交付时明确说明原因和风险。
